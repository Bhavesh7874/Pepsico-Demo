...
export class UserDashboardComponent implements OnInit {
  ...
  loadData() {
    if (this.activeSection === 'home') {
      ...
      this.favoriteService.getMyFavorites().subscribe(favs => this.favorites = favs);
    }
  }
  ...
  async checkout() {
    if (this.cartItems.length === 0 || !this.stripe || !this.card) return;
    ...
    try {
      ...
      this.orderService.createPaymentIntent(total).subscribe({
        next: async (res) => {
